...
void Editor::CreateGroup(const std::string& groupName)
{
	// 指定名のオブジェクトがなければ追加する
	datas_[groupName];
}

void Editor::CreateSection(const HierarchicalName& names)
{
	// なければ追加
...
//}
//
//void Editor::LoadFile(const HierarchicalName& names)
//{
//	// 読み込むJSONファイルのフルパスを合成する
//	std::string filePath = kDirectoryPath + names.kGroup + ".json";
//	// 読み込み用ファイルストリーム
//	std::ifstream ifs;
//	// ファイルを読み込み用に開く
//	ifs.open(filePath);
...
//
//	// グループを検索
//	json::iterator itGroup = root.find(names.kGroup);
//
//	// 未登録チェック
...
int32_t Editor::GetIntValue(const HierarchicalName& names, const std::string& key)
{
	...
	//  グループの参照を取得
	//Group& group = datas_[names.kGroup];
